...
void State_Update(FlightState_t* state, float current_roll, float current_pitch, float dt, uint8_t rc_connected){
	if(state->mode == STATE_ARMED){
		...
		if(rc_connected == 0){
			state->mode = STATE_FAILSAFE;
			state->fail_reason = FAIL_RC_LOST;
		}
	}
	else{
		return;
...
void State_Arm(FlightState_t* state, uint8_t rc_connected, float throttle){
	if(state->mode == STATE_DISARMED && rc_connected == 1 && throttle < 1050.0f){
